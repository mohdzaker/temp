...
const claimPromoCode = async (req, res) => {
  try {
    ...
    if (!promo) {
      return res
        .status(400)
        .json({ status: "failed", message: "Invalid promo code" });
    }
    ...
    if (new Date() > promo.expires) {
      return res
        .status(400)
        .json({ status: "failed", message: "Promo code has expired" });
    }
    ...
    if (promo.total_claimed >= promo.total_users) {
      return res
        .status(400)
        .json({ status: "failed", message: "Promo code limit reached" });
    }
    ...
    if (!claim) {
      return res
        .status(500)
        .json({ status: "failed", message: "Failed to claim promo code." });
    }
    ...
    if (!updatedPromo) {
      return res
        ...
        .json({
          status: "failed",
          message: "Failed to update promo code claim count.",
        });
    }
    ...
    if (!updatedUser) {
      return res
        .status(500)
        .json({ status: "failed", message: "Failed to update user balance." });
    }
